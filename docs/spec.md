...
- **The backdrop** is `config.deckBackdrop`, null by default and drawing nothing. With one, what is
  ...
  handle on how much of it there is. Both follow the expansion, scaled by `speed` and cut to 1, so
  a collapsed deck draws neither and a `speed` above 1 is there before the deck is. A `blur` of 0
  draws no filter and a `dim` of 0 no cover, so the default draws the blur alone.
