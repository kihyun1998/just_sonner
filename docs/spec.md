...
| **controller** | Owns the toast list, ids and timers. No widgets |
...
abstract final class JustSonner {
  /// Mount mode 1 — insert the host into the root overlay of [navigatorKey].
  static void attach(GlobalKey<NavigatorState> navigatorKey, {SonnerConfig config});

  static ToastId show(String title, {
    String? description,
    ToastType type = ToastType.normal,
    Duration? duration,          // null → config.duration; Duration.zero → stays until dismissed
    bool dismissible = true,
    ToastAction? action,
    ToastId? id,                 // an id that is on screen → this call updates it
    ToastBuilder? builder,       // replaces the look for this toast only
  });

  static ToastId success(String title, {String? description, Duration? duration, ToastId? id});
  static ToastId info(String title, {String? description, Duration? duration, ToastId? id});
  static ToastId warning(String title, {String? description, Duration? duration, ToastId? id});
  static ToastId error(String title, {String? description, Duration? duration, ToastId? id});
  static ToastId loading(String title, {String? description, ToastId? id});   // no timer

  /// Changes a toast on screen. Returns false if it is gone.
  static bool update(ToastId id, {String? title, String? description, ToastType? type,
      Duration? duration, ToastAction? action});

  static Future<T> promise<T>(Future<T> future, {
  ...
  static void dismiss(ToastId id);
  static void dismissAll();

  static SonnerController get controller;
}

/// For tests, or an app that wants its own instance.
class SonnerController extends ChangeNotifier {
  SonnerController({SonnerConfig config, Clock? clock});
  // same operations as above, as instance methods
}
...
`SonnerHost` (mount mode 2) and `SonnerConfig` carry: `position`, `width` (356), `gap` (14),
...
- Updating a toast keeps its place in the deck and its enter animation; only its content changes.
...
- `promise` shows `loading` immediately, then updates the same id with `success` or `error`; the
  future's own result or error is returned unchanged to the caller.

## 5. Mounting

| Mode | Setup | Behaviour |
|---|---|---|
| **1. Root overlay** (default) | `JustSonner.attach(navigatorKey)` once, e.g. after `runApp` | The host is inserted into `navigatorKey.currentState!.overlay` on the first toast |
...
| Update | content cross-fades; no re-enter | 200 ms | just_sonner |
...
- `holdTimer()` exists for builders that run their own gestures (flash's `FlashBar` calls
  `deactivate` when a fling starts): it pauses that toast until it is dismissed or updated.
...
- `update` changes content and keeps position; returns false for an unknown id
...
- `promise` success and failure both update the same id and return the future's own outcome
...
1. Facade name — `JustSonner.show(...)`, or a top-level `toast(...)` like sonner?
