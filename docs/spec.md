...
- **Toasts that do not fit scroll**, between the edge and the **cap**, with the mouse wheel or a
  ...
  - **A click past the cap and the far edge's `offset`** reaches the app, not a toast laid out there.
    The near end is a **paint** cut only: a click in the `offset` band is the deck's, before and
    after a scroll, since a deck that can scroll and is held by the pointer stretches its box to
    the layer's edge (the hover region, below) so it cannot slide out from under a resting pointer.
...
- The hover region is the box around the toasts in the deck — the window, or every toast while the
  pointer is over it or the zone is open — and the empty card, gaps included and cut to the layer, and from the edge to the far edge's
...
- A capped deck reaches its cap in pixels, as a share or as a number of toasts, at the top and the
  ...
  honoured; a click past the cap and its margin reaches the app; the pointer holds the deck up to
  that margin; an `expandByDefault` deck is cut with no pointer on it
- A scrolled deck draws nothing nearer than the edge's own `offset`, at the top, the bottom and a
  ...
  exiting toast pinned on screen and the backdrop are cut at the near end too; a click in the
  `offset` band is the deck's, before and after a scroll
