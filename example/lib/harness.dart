...
/// The harness as a page over the showcase, following [settings] as they
/// change.
Route<void> harnessRoute(ExampleSettings settings) => MaterialPageRoute(
  builder: (context) => ListenableBuilder(
    listenable: settings,
    builder: (context, _) => HarnessPage(
      controller: settings.controller,
      config: settings.config,
      onConfig: (value) => settings.config = value,
      themeMode: settings.themeMode,
      onThemeMode: (value) => settings.themeMode = value,
      titleBar: settings.titleBar,
      onTitleBar: (value) => settings.titleBar = value,
    ),
  ),
);

/// The desktop harness: one button per behaviour that exists, so each can be
/// felt rather than only tested.
///
...
class _HarnessPageState extends State<HarnessPage> {
  ...
  @override
  void didUpdateWidget(HarnessPage old) {
    ...
    return [
      ...
      if (control != null) ...[
        ...
        SwitchListTile(
          ...
          onChanged: (on) =>
              setControl(control.copyWith(builder: () => on ? ownHide : null)),
        ),
      ],
      ...
      if (empty != null) ...[
        ...
        SwitchListTile(
          ...
          onChanged: (on) =>
              setEmpty(empty.copyWith(builder: () => on ? ownEmpty : null)),
        ),
      ],
    ];
  }
    ...
    return [
      ...
      if (control != null) ...[
        ...
        SwitchListTile(
          ...
          value: control.countLabel != null,
          onChanged: (on) =>
              set(control.copyWith(countLabel: () => on ? koreanCount : null)),
        ),
        SwitchListTile(
          contentPadding: EdgeInsets.zero,
    ...
    _Section(
      ...
      children: [
        _Button('A look of your own', () {
          _show('Deployed', duration: null, builder: ownLook);
        }),
        _Button('A tall one in your look, then a short one', () {
          ...
          _show('Rolled back', duration: null, builder: ownLook);
        }),
        _Button('Change its look in place', () {
          final id = _show('Changing look', duration: null, builder: ownLook);
          Timer(const Duration(seconds: 1), () {
            if (mounted) _toast.update(id, builder: flashBar(swipe: false));
          });
        }),
        _Button(
          config.builder == null
          ...
          () => widget.onConfig(
            config.copyWith(
              builder: () => config.builder == null ? ownLook : null,
            ),
          ),
        ),
